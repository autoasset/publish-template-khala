...
class SVGCoverter {
    ...
    run(coverter, file, name) {
        return __awaiter(this, void 0, void 0, function* () {
            const result = svgo_1.optimize(yield fs.readFile(file), {
                path: file,
                plugins: [
                    "removeDoctype",
                    "removeXMLProcInst",
            ...
            const path = FilePath_1.default.path(config_1.default.outputs.svg, name);
            yield fs.writeFile(path, result.data);
            yield coverter.pdf(coverter, path, name, data);
            yield coverter.vectordrawable(coverter, path, name, data);
            yield coverter.iconfontGlyphs(coverter, path, name, data);
        });
    }
