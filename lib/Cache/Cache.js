...
module.exports = class Cache {
    ...
    cacheFilePath(key, option) {
        return FilePath_1.default.filePath(this.path, `${key}@${option}`);
    }
    value(key, option) {
        return __awaiter(this, void 0, void 0, function* () {
            const path = this.cacheFilePath(key, option);
            if (yield FilePath_1.default.exists(path)) {
                return path;
            }
            return undefined;
        });
    }
    cache(buffer, key, option) {
        return __awaiter(this, void 0, void 0, function* () {
            if (buffer) {
                yield FilePath_1.default.write(this.cacheFilePath(key, option), buffer);
            }
            else {
                yield FilePath_1.default.delete(this.cacheFilePath(key, option));
            }
        });
    }
    ...
    useCacheByKey(key, option, output, cacheFileBlock) {
        return __awaiter(this, void 0, void 0, function* () {
            const cacheKey = key;
            const newIcon = yield this.value(cacheKey, option);
            if (newIcon) {
                const data = yield FilePath_1.default.data(newIcon);
                if (data) {
                    yield FilePath_1.default.write(output, data);
                }
                return;
            }
            cacheFileBlock(() => __awaiter(this, void 0, void 0, function* () {
                yield this.cache(yield FilePath_1.default.data(output), cacheKey, option);
            }));
        });
    }
};
